...
|---|---|---|---|
| **OPS.52** | 🔑 **R PER BUCKET, AND THE HEADLINE STOPS RANKING TWO ENGINES AGAINST EACH OTHER IN DOLLARS.** | dtp r428 | ✅ **DONE.** Operator on the TEST boxes: *"the 2 test boxes are trading a 10% nominal position size... When they flip to live it's going to be small dollar amounts (that's why we chose them) to observe fill behavior, live position management & broker exits. The P&L will be minuscule, but the R-multiple is what I'll be looking at."* 🔴 **RANKING BY NET $ ACROSS LINEAGES WAS ACTIVELY MISLEADING.** At 10% nominal a TEST bucket can never be best or worst in dollars — it sits near zero and reads as *nothing happening*, which is the opposite of the truth. `best/worst strategy` is now computed **WITHIN** each lineage and labelled `[MAIN]` / `[TEST]`, and when both are present the headline says outright that cross-lineage dollar comparison measures position size rather than strategy. 🔑 **R IS SIZE-INVARIANT, AND THAT IS NOW PROVEN RATHER THAN ASSERTED.** P&L and risk both scale with contracts, so `pnl/risk` is untouched by the sizing — **G4 drives the SAME trades at 1x and 10x contracts and demands an identical R** (measured +0.167 both ways). If that ever diverges, every TEST-vs-MAIN read taken from R is void, which is why it is a gate and not a comment. 🔴 **R IS GROSS OF FEES BY RULING, AND THE RULING IS THE ARTEFACT.** Operator: *"We have to leave fees out because that distorts small contracts. In some cases, the fees will exceed the contract values simply because it's such small numbers, but the R value shouldn't be diminished because of this."* ⚠️ **THIS LOOKS LIKE A BUG AND IS NOT**, so it is written into the module header AND pinned by G1: on a $13–17 underlying with $0.50 strikes options price in cents — **measured, fleet median entry premium $1.48 and the bottom band medians $0.39** — while per-contract fees are FIXED. Netting them would make R a measure of the price point rather than of the strategy, and these boxes exist to observe fills and management, not tick size. **Fees stay visible in their own column; they simply never enter R.** 📊 **AND `pnl_usd` IS GROSS BY CONSTRUCTION, VERIFIED NOT ASSUMED** — it matches pure premium math to the cent on every row of 2026-09-24. 🔑 **THE R COLUMN ALREADY CHANGES THE ANSWER.** Over 2026-09-15..24: by dollars `MAIN/GEXB` leads at +$1,053.50, but **by R `MAIN/SWPT` is the best at +0.531** while `MAIN/RWAY` turns +$806.50 across **97** trades into only **+0.045**. Same data, different conclusion — which is the whole reason the column exists. ⚠️ **AN UNMEASURABLE R PRINTS `-`, NEVER `0.000`** (§0.5) and a thin basis is flagged `~`, because zero R and *we could not measure R* are different facts. `r_n` travels with `r_gross` for the same reason. 🔒 **GATE** — new `tests/check_r_excludes_fees.py` **G1–G4, all four born red** at 3d4bd28. **TWO MUTATIONS, EACH REDDENING EXACTLY ONE CHECK**: netting fees into R reds **G1**; making R scale with contracts reds **G4**. **G1 and G3 are DECLARED REGRESSION GUARDS, not born-red findings** — nothing nets fees today and saying so matters, since a gate that passed the moment it was written is evidence of intent rather than of a bug fixed. ⚠️ **THREE DEFECTS IN MY OWN GATE, FOUND BY RUNNING IT:** (1) **G1's first cut was a regex and it matched `_bucket_r`'s OWN DOCSTRING** — the one explaining *GROSS OF FEES* — so **the documentation of the ruling tripped the gate enforcing the ruling**. That is the **fifth checker self-match recorded this session** (r420 P2, r417 §20, r426 P5, r423's creep predicate); it now asks the AST what the CODE calls. (2) An unguarded probe **crashed the whole gate** at HEAD on a missing helper, reporting only G1 and leaving G2–G4 unstated — *the gate errored* and *the check failed* are different findings. (3) It then **reported G4 twice**, and a gate that double-counts its own failures has untrustworthy totals. ⚠️ **AND MY FIRST SIZE MUTATION WAS A BAD TEST, NOT A PASSING GATE** — dividing by `n` when both samples have n=2 changes nothing, so it proved nothing; the replacement multiplies by total contracts and reds G4 alone. 📊 **SWEEP:** 73/11 → **74/11, identical red set.** **dtp ships nowhere, so COMMIT ONLY; the row is `docs/` so the otv4 half is COMMIT PLUS A BAKE.** |
...
**v3.38 — 2026-09-25 — dtp r428 — R PER BUCKET, AND NO MORE RANKING TWO
ENGINES AGAINST EACH OTHER IN DOLLARS.** [[OPS.52]] NEW.
🔴 At 10% nominal a TEST bucket can never place on dollars — it reads as
"nothing happening", the opposite of the truth. best/worst strategy is now
ranked WITHIN each lineage and says why.
🔑 R is size-invariant, and G4 proves it: the same trades at 1x and 10x give an
identical +0.167. If that diverges, every TEST-vs-MAIN read is void.
🔴 R is GROSS OF FEES by the operator's ruling — it looks like a bug and is
not, so it is in the header and pinned by G1. On a $13–17 underlying options
price in cents (median premium $1.48, bottom band $0.39) while per-contract
fees are fixed; netting them measures the price point, not the strategy.
📊 The column already changes the answer: by dollars GEXB leads at +$1,053, but
by R SWPT is best at +0.531 and RWAY's +$806 over 97 trades is only +0.045.
⚠️ Three defects in my own gate, found by running it: a regex that matched the
docstring documenting the ruling (fifth self-match this session), an unguarded
probe that crashed the gate instead of reporting, and a double-counted G4.
