...
| **ORB.2** | **The resting offer.** One limit at mark for the full geometry count, placed once, standing until FULLY FILLED or the sequence makes it ineligible. | ⬜ | Operator, 2026-08-30: *"leave it as a standing offer until it gets fully filled, or the trade sequence makes it ineligible."* Cancel on: fully filled — past the **50% TP** — **structure stop broken** (same 1m-close test the exit engine uses, cancelled on the SAME tick as the exit) — EOD flatten as backstop. **11:30 gates PLACING, not resting** — operator: *"1130 is the cutoff for new orb entries or a single attempt runaway that has not yet fired during the session"* — so an offer placed in-window keeps accreting after it. 🔑 **SMALLER THAN IT LOOKED:** single-leg entry ALREADY posts a limit at the mark and lets it sit (v3.8). The gaps are only that `confirm_order_fill` cancels at a 20s deadline (`LIVE_ENTRY_DEADLINE_SECONDS`), that the strategy then re-signals against a fresh mark (an effective re-bid, which is ruled out), and that paper returns the full qty in one pass. Needs: restart-proof order state (r179 precedent, trades.db not in-process), a supervisor on BOTH loop branches (an unfilled offer has no position, so the manage branch never runs), honest paper resting fill. ⚠️ MY CALL, VETO IT IF WRONG: a `close_inside` re-arm cancels the standing offer — otherwise a new attempt can confirm at a DIFFERENT strike while the old offer rests, giving two live offers. |
| **ORB.3** | ORB entry window 11:00 — 11:30. | ⬜ | Lands EXACTLY on the debit block (both tests `>=`, ORB declares `long_debit`, `DEBIT_DIRECTIONAL_CUTOFF_ET` is 11:30) — no gap, no overlap. ⚠️ Not only a later deadline: the constant also sets `entries_expired` and expires the engine from ANY state, and r60 made the re-arm check read it, so it buys 30 more minutes of breaks, retests and re-arms. Two stale copies: `tests/cascade_harness.py:38`, `tests/cascade_real.py:45` — plus a test pinning every copy to the config value. |
| **ORB.4** | Pool in path becomes RECORD-ONLY. | ⬜ | Detection and the journal/plan rows stay; the target pull at `orb_strategy.py:482-485` (consumed at :232) goes. Target returns to pure measured move. |
