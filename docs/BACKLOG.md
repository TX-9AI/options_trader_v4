...
| **ORB.2** | **The resting offer** — DESIGN SETTLED 2026-08-30, three surfaces. | ⬜ | Operator: *"broker-side open positions [are] the only thing that can declare an open position in the live book. That can be simulated in paper. If it also discloses open bids/asks, that might be good information to have separately but not on the open positions (trades) log."* 🔑 **(1) PLACEMENT is a logged EVENT, never a position. (2) OPEN POSITIONS are a projection of the broker's positions endpoint, grouped by strike+type — discovered, not declared. (3) WORKING ORDERS are their own informational surface, kept OFF the trades log.** — MOST OF IT EXISTS: `get_open_option_positions()` already returns `{symbol, underlying, quantity, direction, average_open_price}`, its docstring already says *"the brokerage is the source of truth for whether a position exists"*, and **the broker already averages the basis across partials** — so accretion needs no arithmetic of ours and no persisted remaining-quantity. ⚠️ THE HAZARD: `_intraday_reconcile` deliberately *"does not adopt brand-new broker positions intraday, so a manual trade you place is left alone."* Adoption must be scoped to a strike/side the bot holds a WORKING ORDER for — which is what makes surface (1) load-bearing. Paper gets a simulated positions surface so the shape stays common. |
