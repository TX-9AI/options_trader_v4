...
| **S3.4** | Menu **Excursion report** vs **Stop / TP sweep** — **MY RECOMMENDATION: RETIRE THE EXCURSION REPORT, KEEP THE R SUITE, AND REBUILD THE ONE THING ONLY IT MEASURES.** Operator's call. | ⬜ | They are not duplicates, and the v4 suite is wider than I first said. **`r_ledger.py` already computes capture and giveback per strategy / per option side / per exit reason** — that is `excursion_report`'s descriptive core, v4-native and S3-default. `stop_sweep` owns the R surface over the two extremes; `exit_replay` owns the whole premium path, so trail arm level and giveback width are measurable rather than argued. Three tools, three questions, all v4. **The ONE thing none of them measures is `NEVER_FAVORABLE_CUTS`** — the split between trades that were never favourable at all (a SELECTION problem) and winners that gave gains back (an EXTENSION problem). Those have different fixes and a stop sweep cannot tell them apart. So: retire the 49 KB of v3 accumulation, and add the never-favourable split to `r_ledger`, which already holds the population and the capture number. Preserves the intent, salvages none of the code. |
...
| **RPT.2** | **`r_ledger.py` has no menu item.** | ⬜ | It runs only inside the nightly conductor and Telegrams a headline, so the R baseline — R, expectancy, capture, giveback — cannot be asked for on demand. `_r_tool` already exists and takes a tool filename, so this is one registry line. It also becomes the natural home for S3.4's never-favourable split. |
...
| **C.16** | The **S3 WAREHOUSE** section still reads *"(read-only; runs ALONGSIDE the local reports)"*. After r184–r187 there is barely a local report left to run alongside. Same stale framing MENU.1 removed from R SUITE; left deliberately for a separate call. | r188 |
