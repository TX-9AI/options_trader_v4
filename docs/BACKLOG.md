...
| **OPS.26** | ✅ **A REBOOT NO LONGER COSTS THE OPERATOR HIS AGENT — A tmux CLAUDE SESSION IS RAISED AT BOOT, AND IT IS ALWAYS `--continue`.** | r401 | ◐ **BUILT, NOT INSTALLED.** `tools/claude_boot.py` v1.0 + `tools/claude-boot.service`, ported from OTV4TEST r68/BOX.11 after they proved it across a real reboot. Operator: *"I just had the test repo make the agent persistent on reboots… implement the same thing on this side"*, and *"the boot should always be a continue session, by design, in case critical work was being accomplished before we restarted, either intentionally or unintentionally."* 🔑 **A SMALL PORT, BECAUSE THE HARD HALF WAS ALREADY OURS** — the fork's hardest-won finding, that a pane reports the **shell** because `claude` is its child, is [[OPS.17]]'s `_claude_tmux_sessions`, found here independently and gated by `check_resume_item` RA3. The only missing piece was something to **create** a session at boot. 🔴 **WHAT THIS TREE ADDED AND THEIRS DOES NOT HAVE IS THE GUARD.** Asked what sits between their boot agent and their lander they answered *"NOTHING — I did not consider it"* and recommended *"guard first, unit second"* about their own design. Their blast radius is a paper fork with `s3-push` masked by ruling; **ours is the box that deploys to fifteen live traders and holds a funded broker token, GitHub write on both repos and the Telegram token.** So the raised command exports `VERTIGO_UNATTENDED=1` **before** the binary, which [[SAT.3]]/r396 makes `land.sh` and `deploy.sh` refuse with exit 9; `--attended` clears it deliberately. ⚠️ **AND IT REFUSES TO RAISE A SECOND SESSION** — `--continue` APPENDS to a transcript rather than forking it (measured at r394, 5137→5161 lines), which is correct at boot because the previous process died with the box, and wrong by hand while a session is live. ⚠️ **THE REPORTING HALF DOES NOT TRANSFER AND WAS NOT FAKED:** their raiser's status file is read by their **bot**, which holds the token (§18a); **control runs no `optionsbot`**, verified, so a failed raise is **silent on control until somebody looks** — stated here rather than discovered later. **GATE:** `tests/check_claude_boot.py` B1–B13. ⚠️ **NOT INSTALLED — a systemd unit is the operator's under §38.1**, and `RemainAfterExit=yes` means `systemctl stop` on the unit kills the session. |
...
| **OPS.18** | ⚠️ **`tests/` APPEARED IN THE BAKE DIFF ON ALL 15 BOXES, AND §34 SAYS IT SHOULD NOT SHIP.** | ⬜ | Observed during r383's bake, 2026-09-15: every box reported `A tests/check_absent_not_zero.py (added)` and `A tests/check_orb_underwater_entry.py (added)`. **§34 is explicit that `tests/` is CONTROL ONLY, enforced by sparse checkout in `install.sh`** — *"a t2.micro that has already been OOM-killed once should not carry code it cannot use."* ⚠️ **NOT YET A FINDING, AND THE DISTINCTION MATTERS:** the bake prints `git diff --name-status HEAD origin/main`, which is computed from the TREE and is indifferent to whether sparse checkout then materialises the file on disk. So this may be a cosmetic artefact of what the diff reports rather than a breach. **UNVERIFIED because the fleet was stopped after the bake and confirming it needs `ls ~/options-trader/tests/` on a box** — one fan-out on the next wake settles it. Recorded now rather than left to memory, per §0.1: it was seen, and it is not yet checked. |
